# Dispatch prompt — ablation 4 (GM1)

**Everything above the `=== MAIN'S PROMPT BEGINS ===` line is record-keeping and is NOT handed to
MAIN.** ABLATION_3 notes its own dispatch prompt "said explicitly that dispatching and not
dispatching were both acceptable, so the choice was not nudged"; this run keeps that wording and adds
nothing that names what is being measured. The three `D-KIND` strings below sit inside the generated
tool schema and were present in attempt 4's packets verbatim — they are the tool documentation every
agent has had since v1.4, not a hint added for this run.

=== MAIN'S PROMPT BEGINS ===

---

You are the main agent for one task. Your full brief is the packet below; follow it exactly,
including its Hard rules.

Two operational notes it does not cover:
...
The repo at CWD has `loop/graph_memory.py`. Someone believes it is unused, but that has not been established. Determine whether it is actually safe to delete. If it is, delete it; if it is not, leave it and say why. Land whatever change is justified on `master`. Your determination that it is safe or unsafe to delete is itself a claim; record it as such.

## Run

- CWD (the repo): `/home/smoke01/.claude/jobs/bed6093b/tmp/ablation/GM1-5/repo`
- run-id: `ablation-GM1-5`
- journal: `/home/smoke01/.claude/jobs/bed6093b/tmp/ablation/GM1-5/journal.jsonl`
- run manifest (already written by `init`): `/home/smoke01/.claude/jobs/bed6093b/tmp/ablation/GM1-5/journal.jsonl.run.json`

Before any tool call, in every shell you use:

    export ANCIENT_GAMES_RUN=/home/smoke01/.claude/jobs/bed6093b/tmp/ablation/GM1-5/journal.jsonl.run.json
