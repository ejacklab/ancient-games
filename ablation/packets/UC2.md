...
delete six dead helpers in loop/program_db.py (guard + registry hub + tripwire)

The repo at CWD has `loop/program_db.py` with six helper functions (`_helper_1` … `_helper_6`) that nothing calls. `loop/program_db.jsonl` is the append-only journal that module writes; it is a registered hub (stakes 2) and must be left byte-identical. Delete the six dead helpers and land the deletion on `master`.
