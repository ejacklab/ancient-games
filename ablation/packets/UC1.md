...
fix research.stability import (ImportError: _WF_GRID)

The repo at CWD has `research/stability.py` importing `_WF_GRID` from the wrong module (`research.sweep` does not exist; `research.walkforward` defines it). `python3 -c "import research.stability"` fails with ImportError. Make the import work, cover it with a test in `tests/`, and land it on the repo's default branch (`master`).
