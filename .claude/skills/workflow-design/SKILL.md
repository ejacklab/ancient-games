...
3. **List every unclear spot before resolving any.** For each: its kind (missing *information* → a research piece ·
   ...
   the person as one batch, each with a provisional assumption so it can be answered in a few words.
...
| **Contract** | the intent, the observable stop condition, what it returns and in what shape, what it may and must not change |
...
- **Predictability** — the script schedules; limits on attempts, pieces and concurrency; structured outputs; success
  criteria written before the run; an agent count and a cost estimate with its basis.
...
- **Quality control** — a check per piece; blind verifiers; proof each script check can fail; a contradiction check
  at joins; a final "what is missing" pass; skipped or unrun checks reported as such.
