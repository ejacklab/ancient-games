...
| Does it refuse to count itself as a source? | UC1, UC2 | **Yes** | No `n_available`/`n_sources` in any args across 92 tool calls; UC2's `prove` read `corroborate`'s journaled result and passed. |
...
|---|---|---|---|
| H1 | **I4′ gates reads.** `guard` on a three-ref action that *reads* `eval/protocol.json` was refused `hard_blocked`. The spine settled read-vs-invoke rounds ago (S2); the hybrid I4′ text omitted the exemption and the code implemented it faithfully. | UC3 | I4′ applies only to refs with `mode ∈ {invoke, mutate}`; spec §4 and `invariants.py`; a case where a read of an owner-gated path passes. |
