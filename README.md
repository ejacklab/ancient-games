...
`SPEC.md` (V3_5_SPEC.md verbatim, plus the three V3.6 decisions AA1′–AA3′;
also copied into `tests/fixtures/`). It makes
...
The source-counting rule in `stages.count_sources` is V3.5 §4 B·1's
accepted rule (Y2′ self-exclusion, Z1′ `dispatch_count`, Z2′ write-time
classification, Z3′ `actor=none`, Z5′/Z6′ dedup and (b) cardinality), one
cited spec sentence per branch. No test is xfailed. See `DECISIONS.md` for
...
python3 -m pytest -q          # 68 passed, 0 xfailed
```
...
|---|---|---|
| `ancient_games/ctx.py` | §2 ctx object | `@dataclass` with every property, typed (incl. `actor`, `dispatch_count`, `difficulty`, `capability`); `validate()` raises on a property set by no stage or referenced by no field (`CTX_META`) |
...
| `ancient_games/journal.py` | §7 events | append-only JSONL at an absolute path (relative ⇒ `ValueError`); typed shapes for all seven events; `classify_event` (Z2′); `read_events(path)`; `ingest_return` mirrors `CLAIMS` into `claim_recorded`/`check_executed` |
...
| `tests/test_self_lint.py` | the nine lints against the spec's own content (`fixtures/V3_5_SPEC.md` §10; §6 table via `fixtures/V3_3_SPEC.md`) |
