...
All commands below were run in this session on 2026-09-20 at HEAD `edd0edc`, from `/home/smoke01/dev/ancient-games`.
...
| Earlier recorded baselines | `20260919-plan-critic-issues.md:8` (379 passed, 0 failed, 0 skipped); Claude Code auto-memory `ancient-games-house-rules.md` (379 passed at `edd0edc`) | md file; memory folder | Yes — both agree with this session's run | The md file: any agent. The auto-memory: Claude Code only |
...
|---|---|
| The count is a command result, not stored knowledge: it changes with every added test and with the `NO_COLOR` environment variable, so the 379 recorded here is dated 2026-09-20 / `edd0edc` and must be taken again by whoever edits the README, in the same session as the edit | verify piece |
